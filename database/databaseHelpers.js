...
var createPokemonInstance = (pokemonId, username, name, level, cb) => {
  getUserIdByUsername(username, (err, userId) => {
    if (err) {
      console.log(err);
    } else {
      console.log('userID:', userId);
      db.query('INSERT INTO pokemon_instance (pokemon_id, user_id, name, level) VALUES (?, ?, ?, ?)', [pokemonId, userId, name, level], (err, results) => {
        cb(err, results);
      });
    }
  });
